...
beforeAll(async () => {
    // 初始化全局临时目录
    global.__TEST_TEMP_DIR__ = await fs.mkdtemp(join(tmpdir(), TEST_CONFIG.tempDirPrefix));
    // 初始化清理函数数组
 ...
 * 创建测试项目结构
 */
export async function createTestProject(dir, type = 'typescript') {
    await fs.mkdir(join(dir, 'src'), { recursive: true });
    switch (type) {
        case 'typescript':
            await fs.writeFile(join(dir, 'src/index.ts'), 'export const hello = (name: string) => `Hello, ${name}!`');
            ...
            break;
        case 'vue3':
            await fs.writeFile(join(dir, 'src/index.ts'), `import { defineComponent } from 'vue'
export default defineComponent({
  ...
  template: '<div>Hello Vue 3!</div>'
            ...
            break;
        case 'style':
            await fs.writeFile(join(dir, 'src/index.css'), `.test { color: red; }`);
            break;
    }
 ...
 * 模拟构建器适配器
 */
export function createMockAdapter(name = 'rollup') {
    return {
        name,
        version: '4.0.0',
        available: true,
