...
export class Vue3Strategy {
     ...
     */
    async applyMultiFormatStrategy(config) {
     ...
     */
    buildMultiFormatOutputs(config, multiFormat) {
        const outputs = [];
     ...
     */
    async buildMultiFormatPlugins(config, cssStrategy) {
        const plugins = [];
        try {
            // Vue SFC 插件
            const vuePlugin = await import('rollup-plugin-vue');
            ...
            if (config.multiFormat?.es || config.multiFormat?.esm || config.multiFormat?.lib) {
                try {
                    ...
                    plugins.push(multiInput.default({
                        relative: 'src/',
                        transformOutputPath: (output) => output
                    }));
                }
                catch (e) {
     ...
     */
    async buildCSSStrategyPlugins(config, cssStrategy) {
        const plugins = [];
        switch (cssStrategy) {
            case 'single':
                // CSS 打包到一个文件中
