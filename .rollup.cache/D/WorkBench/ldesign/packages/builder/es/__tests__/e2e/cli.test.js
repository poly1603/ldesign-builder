...
describe('CLI End-to-End Tests', () => {
    ...
    describe('Build Command', () => {
        it('should build TypeScript library successfully', async () => {
            ...
            await fs.writeFile(join(tempDir, 'src/index.ts'), 'export const hello = (name: string) => `Hello, ${name}!`');
        ...
        it('should handle build errors gracefully', async () => {
            ...
            await fs.mkdir(join(tempDir, 'src'), { recursive: true });
            await fs.writeFile(join(tempDir, 'src/index.ts'), 'export const hello = (name: string => `Hello, ${name}!` // Syntax error');
            // Run CLI build command
            const result = await runCLI(['build'], tempDir);
    ...
    describe('Watch Command', () => {
        it('should start watch mode', async () => {
            // Create test project
            await fs.mkdir(join(tempDir, 'src'), { recursive: true });
            await fs.writeFile(join(tempDir, 'src/index.ts'), 'export const hello = () => "Hello, World!"');
