...
describe('TypeScriptStrategy.resolveInputEntries', () => {
    ...
    it('should return user-provided input when specified', async () => {
        ...
        const result = await strategy.resolveInputEntries(config);
    ...
    it('should fallback to src/index.ts when no files found', async () => {
        ...
        expect(mockFindFiles).toHaveBeenCalledWith(['src/**/*.{ts,tsx,js,jsx}'], expect.objectContaining({
            cwd: process.cwd(),
    ...
    it('should use correct glob patterns and ignore rules', async () => {
        ...
        mockFindFiles.mockResolvedValue([]);
        await strategy.resolveInputEntries(config);
        expect(mockFindFiles).toHaveBeenCalledWith(['src/**/*.{ts,tsx,js,jsx}'], {
            cwd: process.cwd(),
            ignore: [
                '**/*.d.ts',
                '**/*.test.*',
...
//# sourceMappingURL=multi-entry.test.js.map