...
describe('examples command', () => {
    ...
    it('should find example projects with package.json', async () => {
        ...
        const findExampleProjects = examplesModule.findExampleProjects;
    ...
    it('should run example build successfully', async () => {
        const examplesModule = await import('../../../cli/commands/examples');
        const runExample = examplesModule.runExample;
        const mockChild = {
        ...
        expect(mockSpawn).toHaveBeenCalledWith(process.execPath, ['/bin/ldesign-builder.js', 'build'], {
            cwd: '/project/path',
            stdio: 'inherit',
            env: process.env,
        });
