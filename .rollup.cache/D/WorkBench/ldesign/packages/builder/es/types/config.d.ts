 ...
 * 配置相关类型定义
 */
import type { LogLevel, BuildMode, FilePath, ValidationResult, ConfigSchema, CacheOptions, WatchOptions, EnvironmentVariables, KeyValueMap } from './common';
import type { LibraryType, TypeScriptLibraryConfig, VueLibraryConfig, VueJsxConfig, StyleLibraryConfig, LibraryBuildOptions } from './library';
...
export interface BuilderConfig {
    ...
    /** TDesign 风格的多格式构建配置 */
    multiFormat?: MultiFormatBuildConfig;
    /** 样式处理策略配置 */
 ...
 * TDesign 风格的多格式构建配置
 */
export interface MultiFormatBuildConfig {
    /** 是否启用 ES 模块构建（支持 tree-shaking） */
    es?: boolean;
    /** 是否启用 ESM 构建（保留源码样式引用） */
    esm?: boolean;
    /** 是否启用 Lib 构建（库格式） */
    lib?: boolean;
    /** 是否启用 CJS 构建（CommonJS 格式） */
    cjs?: boolean;
    /** 是否启用 UMD 构建（浏览器格式） */
    umd?: boolean;
    /** 是否启用压缩版本的 UMD 构建 */
 ...
 * CSS 构建策略类型
 */
export type CSSBuildStrategy = 'single' | 'multi' | 'source' | 'ignore';
/**
 * CSS 构建策略配置
 */
export interface CSSStrategyConfig {
    /** CSS 构建策略 */
    strategy: CSSBuildStrategy;
    /** 是否提取 CSS 到单独文件 */
    extract?: boolean;
    /** 是否压缩 CSS */
    minimize?: boolean;
