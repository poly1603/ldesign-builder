 ...
 * 增强版库构建器主控制器类
 */
export class EnhancedLibraryBuilder extends EventEmitter {
    constructor(options = {}) {
        super();
        /** 当前状态 */
    ...
    async build(config) {
        ...
        try {
            ...
            if (!configValidation.valid) {
                throw this.errorHandler.createError(ErrorCode.CONFIG_VALIDATION_ERROR, `配置验证失败: ${configValidation.errors.join(', ')}`);
            }
            ...
            if (!functionalityCheck.identical) {
                ...
                if (mergedConfig.postBuildValidation?.failOnError) {
                    throw this.errorHandler.createError(ErrorCode.BUILD_FAILED, '打包前后功能不一致');
                }
            }
            // 结束性能监控
    ...
    async detectCircularDependencies(entry) {
        ...
        const cycles = [];
        const dfs = async (filePath, trace = []) => {
    ...
    async postprocessOutputs(result, config) {
        ...
        for (const output of result.outputs) {
            ...
            const needGzip = (typeof config.performance?.bundleAnalyzer === 'object' && config.performance.bundleAnalyzer.showGzip === true) || !!config.performance?.sizeLimit;
            if (needGzip) {
