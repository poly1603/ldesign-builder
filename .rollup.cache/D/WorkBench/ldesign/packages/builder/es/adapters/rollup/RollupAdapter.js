...
export class RollupAdapter {
    ...
    async transformConfig(config) {
        ...
        if (config.output) {
            ...
            if (outputConfig.esm || outputConfig.cjs || outputConfig.umd) {
                const configs = [];
                const baseOutDir = outputConfig.dir;
                // 处理 ESM 配置
                if (outputConfig.esm && outputConfig.esm !== false) {
                    ...
                    const esmDir = (baseOutDir || esmConfig.dir || 'es');
                    ...
                    const esmInput = esmConfig.input
                        ...
                        : filteredInput;
                    const isMultiEsm = this.isMultiEntryBuild(esmInput);
                    ...
                    configs.push({
                        ...
                        output: {
                            ...
                            preserveModules: isMultiEsm,
                            preserveModulesRoot: isMultiEsm ? 'src' : undefined,
                ...
                if (outputConfig.cjs && outputConfig.cjs !== false) {
                    ...
                    const cjsDir = (baseOutDir || cjsConfig.dir || 'lib');
                    ...
                    const cjsInput = cjsConfig.input
                        ...
                        : filteredInput;
                    const isMultiCjs = this.isMultiEntryBuild(cjsInput);
                    // 统一注入 Banner/Footer/Intro/Outro
                    ...
                    configs.push({
                        ...
                        output: {
                            ...
                            preserveModules: isMultiCjs,
                            preserveModulesRoot: isMultiCjs ? 'src' : undefined,
                            globals: outputConfig.globals,
                            name: outputConfig.name,
                            banner: _bannerCjs,
                            footer: _footerCjs,
            ...
            if (Array.isArray(outputConfig.format)) {
                ...
                for (const format of formats) {
                    ...
                    const dirBase = outputConfig.dir;
                    const dir = dirBase || (isESM ? 'es' : isCJS ? 'lib' : 'dist');
                    ...
                    configs.push({
                        ...
                        output: {
                            ...
                            preserveModules: isMultiEntry,
                            preserveModulesRoot: isMultiEntry ? 'src' : undefined,
                            banner: _banner,
                            footer: _footer,
                            intro: _intro,
                            outro: _outro
                ...
                const isMultiEntryOut = this.isMultiEntryBuild(filteredInput);
                rollupConfig.output = {
                    dir,
                    format: mapped,
                    name: outputConfig.name,
                    ...
                    assetFileNames: '[name].[ext]',
                    exports: isESM ? outputConfig.exports ?? 'auto' : 'named',
                    preserveModules: isMultiEntryOut,
                    preserveModulesRoot: isMultiEntryOut ? 'src' : undefined,
                    banner: _banner2,
                    footer: _footer2,
                    intro: _intro2,
                    outro: _outro2
     ...
     */
    async validateOutputArtifacts(config) {
        try {
            ...
            this.logger.warn(`验证输出产物时出错: ${error.message}`);
            // 出错时保守处理，认为产物不存在
            return false;
        }
    }
     ...
     */
    async createUMDConfig(config, filteredInput) {
        // 处理 boolean 配置
    ...
    async resolveBanner(bannerConfig, config) {
        ...
        if (bannerConfig && bannerConfig.buildInfo) {
            const buildInfo = await this.generateBuildInfo(bannerConfig.buildInfo);
            if (buildInfo)
     ...
     * 生成构建信息
     */
    async generateBuildInfo(buildInfoConfig) {
