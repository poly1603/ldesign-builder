 ...
 * @returns 解析后的文件路径
 */
export async function resolveInputPatterns(input, rootDir = process.cwd()) {
    // 如果是对象（多入口），递归处理每个值
 ...
 * @returns 规范化后的输入配置
 */
export async function normalizeInput(input, rootDir = process.cwd(), exclude) {
    if (!input) {
        // 默认入口
 ...
 * @returns 过滤后的输入配置
 */
async function applyExcludeFilter(resolved, exclude, rootDir) {
    // 如果是字符串，检查是否匹配排除模式
 ...
 * @returns 是否被排除
 */
async function isFileExcluded(filePath, exclude, rootDir) {
    // 将绝对路径转换为相对路径进行匹配
 ...
 * @returns 输出目录列表
 */
export function getOutputDirs(config) {
    const dirs = [];
    // 主输出目录
