...
export class PackageUpdater {
     ...
     * 创建单个 export 条目
     */
    createExportEntry(relativePath) {
     ...
     */
    createWildcardExportEntry(dirName) {
        const { esm, cjs, types } = this.config.outputDirs;
        const entry = {};
        // 类型声明 - 使用 types 目录配置
        if (types) {
     ...
     * 扫描 src 下的直接子目录（不递归）
     */
    async scanDirectDirectories(srcPath) {
        try {
     ...
     */
    sortPackageJsonFields(packageJson) {
        // 定义字段的优先级顺序
        const fieldOrder = [
            // 基本信息
            'name',
     ...
     * 在指定目录中查找CSS文件
     */
    async findCssFiles(dir, cssFiles, prefix) {
        const fs = await import('fs');
        const path = await import('path');
        try {
 ...
 * 创建 Package 更新器的便捷函数
 */
export function createPackageUpdater(config) {
