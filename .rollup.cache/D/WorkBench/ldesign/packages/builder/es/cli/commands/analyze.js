...
function renderMarkdown(report) {
    ...
    if (meta.duration)
        lines.push(`- 耗时: ${formatMs(meta.duration)}  `);
    ...
    if (meta.cache) {
        ...
        if (typeof meta.cache.hit === 'boolean')
            lines.push(`- 命中: ${meta.cache.hit ? '是' : '否'}  `);
        if (typeof meta.cache.lookupMs === 'number')
            lines.push(`- 查询耗时: ${formatMs(meta.cache.lookupMs)}  `);
        if (typeof meta.cache.savedMs === 'number')
            lines.push(`- 预计节省: ${formatMs(meta.cache.savedMs)}  `);
        if (meta.cache.dir)
            lines.push(`- 目录: ${meta.cache.dir}  `);
        if (typeof meta.cache.ttl === 'number')
            lines.push(`- TTL: ${formatMs(meta.cache.ttl)}  `);
        if (typeof meta.cache.maxSize === 'number')
            lines.push(`- 最大体积: ${formatFileSize(meta.cache.maxSize)}  `);
    }
    lines.push('');
    lines.push(`## 总览`);
    lines.push('');
    lines.push(`- 文件数: ${totals.fileCount ?? files.length}`);
    if (typeof totals.raw === 'number')
        lines.push(`- 原始大小: ${formatFileSize(totals.raw)}`);
    if (typeof totals.gzip === 'number')
        lines.push(`- Gzip: ${formatFileSize(totals.gzip)}`);
    lines.push('');
    // Top 10 by gzip/raw
...
function renderHTML(report) {
    ...
    const topLis = top.map((f) => `<li>${escapeHtml(f.fileName)}${f.format ? ` (${escapeHtml(f.format)})` : ''}: ${escapeHtml(formatFileSize(f.gzipSize || f.size))}</li>`).join('\n');
    return `<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
...
function computeDiff(current, baseline) {
    const cFiles = current?.files || [];
    const bFiles = baseline?.files || [];
    const cMap = new Map(cFiles.map((f) => [f.fileName, f]));
    const bMap = new Map(bFiles.map((f) => [f.fileName, f]));
    const common = [];
    const added = [];
    const removed = [];
    // common & added
...
function sum(arr, key) { return arr.reduce((n, x) => n + (x?.[key] || 0), 0); }
function renderMarkdownDiff(current, baseline, opts = {}) {
...
function renderHTMLDiff(current, baseline, opts = {}) {
    const { common, added, removed, totals } = computeDiff(current, baseline);
    ...
    const addedLis = added.map((f) => `<li>${escapeHtml(f.fileName)}${f.format ? ` (${escapeHtml(f.format)})` : ''}: ${escapeHtml(fmtSize(f.gzipSize || f.size))}</li>`).join('\n');
    const removedLis = removed.map((f) => `<li>${escapeHtml(f.fileName)}${f.format ? ` (${escapeHtml(f.format)})` : ''}: ${escapeHtml(fmtSize(f.gzipSize || f.size))}</li>`).join('\n');
    const cCache = current?.meta?.cache;
    const bCache = baseline?.meta?.cache;
...
function fmtSize(n) { return typeof n === 'number' && n >= 0 ? formatFileSize(n) : '-'; }
function formatSignedSize(delta) {
    const sign = delta > 0 ? '+' : '';
    return `${sign}${formatFileSize(Math.abs(delta))}`;
}
function formatSignedMs(delta) {
    const sign = delta > 0 ? '+' : '';
...
function resolveOutputPath(reportPath, output, ext) {
