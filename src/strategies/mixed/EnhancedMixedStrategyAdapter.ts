...
import { EnhancedMixedStrategy, type MixedFrameworkConfig } from './EnhancedMixedStrategy'

/**
 * 增强混合策略适配器
 */
export class EnhancedMixedStrategyAdapter implements ILibraryStrategy {
  readonly name = 'enhanced-mixed'
  readonly supportedTypes = [LibraryType.MIXED, LibraryType.ENHANCED_MIXED]
  readonly priority = 100

  private strategy: EnhancedMixedStrategy

  constructor(config?: MixedFrameworkConfig) {
    this.strategy = new EnhancedMixedStrategy(config)
  }
  ...
  async applyStrategy(config: BuilderConfig): Promise<UnifiedConfig> {
    ...
    if (JSON.stringify(mixedConfig) !== JSON.stringify(this.strategy['config'])) {
      this.strategy = new EnhancedMixedStrategy(mixedConfig)
    }
 ...
 * 创建增强混合策略适配器
 */
export function createEnhancedMixedStrategyAdapter(config?: MixedFrameworkConfig): EnhancedMixedStrategyAdapter {
  return new EnhancedMixedStrategyAdapter(config)
}
