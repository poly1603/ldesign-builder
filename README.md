...
- **构建生命周期钩子**：支持 `beforeBuild`、`afterBuild`、`onError` 等钩子，灵活扩展构建流程
- **自动重试机制**：遇到临时错误自动重试，提高构建稳定性
- **构建历史记录**：记录构建历史，支持构建结果对比分析
- **增强的类型系统**：补充了大量工具类型和类型守卫函数
- **高级构建选项**：支持并行构建、增量构建、内存优化等高级配置
